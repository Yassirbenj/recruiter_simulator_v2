...
def main(job_offer,seniority,type_interview,language,type):
    ...
    if "messages" not in st.session_state:
        st.cache_data.clear()
        personae=config_persona(job_offer,seniority,type_interview,language)
        if personae:
            st.session_state.messages=[
                SystemMessage(content=personae)
                ]
            st.session_state.messages=[HumanMessage(content=f"Hello, I'm available to start the job interview {job_offer}. the job interview will be in {language} language. Can you start with a first question ?")]
    ...
    if prompt := prompt_model:
        ...
        with st.sidebar:
                ...
                evaluation=evaluate_sentence2(job_offer,prompt,language,last_question)
                st.write(evaluation)
...
def config_persona(job_offer,seniority,type_interview,language):

    start=st.button("start")
    if start:
        persona=f'''Your name is John.
                    You are conducting an interview in {language} language.
                    You are a recruiter conducting an interview for the job offer {job_offer}.
...
main(job_offer,seniority,type_interview,language,option)
