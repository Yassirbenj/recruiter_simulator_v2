...
def main(job_title,job_details,seniority,type_interview,language,type):
    ...
    if type=="text" and st.session_state.status=='started':
        st.write("write")
...
if "messages" not in st.session_state:
    ...
    if start:
        ...
        write_messages(option)
        if option=="voice":
            prompt_voice=stxt2(openai_api_key)
...
if "prompt" in st.session_state:
    st.write('continue')
    st.write(f'session state: {st.session_state.prompt}')
    st.session_state.messages.append(HumanMessage(content=st.session_state.prompt))
    with st.spinner ("Thinking..."):
        response=chat(st.session_state.messages)
    st.session_state.messages.append(AIMessage(content=response.content))
    write_messages(option)
    prompt_voice=stxt(openai_api_key)
    if prompt_voice:
        st.session_state.prompt=prompt_voice


    #with st.sidebar:
    #    last_question=st.session_state.get('messages',[])[-2].content
    #    prompt=st.session_state.get('messages',[])[-1].content
    #    with st.sidebar:
    #        st.write(f'question: {last_question}')
    #        st.write(f'reponse: {prompt}')
        #evaluation=evaluate_sentence2(job_title,prompt,language,last_question)
